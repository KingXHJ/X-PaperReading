...
5. Info NCE：
    ...
    - $-log\frac{exp(z_k)}{\sum ^ {k} _ {i=0} exp(z_ {i})}$ 这是cross entropy loss
    ...
    - 但是二分类可能对模型学习不友好，在那么多噪声中，大家可能不是一个类，看成多分类比较好，NCE loss变成了Info NCE loss $$\pounds_q=-log\frac{exp(q\cdot k_+/\tau)}{\sum ^ {K} _ {i=0}exp(q\cdot k_i/ \tau)}$$
        - $\tau$ 是温度的超参数，控制分布形状
